...
parser.add_argument('--cl_step', type=int, default=1, help='steps for curriculumparser.parse_args([]) training')
...
def showIamge(input, idx):
    image_array = input.cpu().numpy()
    # fig, ax = plt.subplot(2, 2, figsize=(6, 6))
    # for i, axi in enumerate(ax):
    #     axi.imshow(image_array[i])
    # plt.show()
...
def addImageNoise(inputs, noise_type):
    im_arr = inputs.cpu().numpy()
    batch_size = im_arr.shape[0]
    if noise_type == 'gaussian':
        aug = iaa.AdditiveGaussianNoise(loc=0, scale=(0.0, 0.001*255), per_channel=True)
    elif noise_type == 'poisson':
        aug = iaa.AdditivePoissonNoise(lam=0.25, per_channel=True)
    elif noise_type == 'salt_pepper':
        aug = iaa.SaltAndPepper(p=0.05, per_channel=True)
    else:
        raise Exception('No this noise type!')

    for i in range(batch_size):
        im_arr[i] = aug.augment_image(im_arr[i])

    im_arr = torch.from_numpy(im_arr)

    return im_arr.permute(0, 3, 2, 1)
